...
* [x] Apply a similar mono-repo approach to `go-openapi/strfmt` which suffer from similar woes: bloated API,
      imposed dependency to some database driver.
* [x] ci pipeline to manage releases
* [x] Adapt `go-swagger` (incl. generated code) to the new `swag` API.
* [x] Benchmark & profiling: publish independently the tool built to analyze and chart benchmarks (`fredbi/benchviz`, similar to `benchvisual`)
* [ ] Factorize some tests, as there is a lot of redundant testing code in `jsonutils`
* [ ] jsonutils: more thorough testing for nil / null case
...
* [ ] Move back to this common shared pot a few of the technical features introduced by go-swagger independently
      (e.g. mangle go package names, search package with go modules support, ...) -> mangling v2, will move to go-openapi/codegen
