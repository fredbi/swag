...
// buf is a minimal append-only byte sink shared by the string-returning verbalizers
// ([cardinal], [spellDecimal], [NumberMangler.NumberWords])
// and the allocation-free [NumberMangler.AppendWords].
//
...
// Grow ensures room for n more bytes, so a verbalized run that expands (e.g. "200" -> "two hundred")
// does not trigger a mid-build reallocation.
