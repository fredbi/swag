...
// Usage:
//
//	go run github.com/go-openapi/swag/mangling/v2/ucd/cmd/gen_asciifold [package [outfile [ucd-dir]]]
//
...
const (
	...
	defaultOutFile     = "asciifold_table.go"
...
func run(pkg, outFile, ucdLocation string) error {
	...
	if err := emit(inFile, pkg, outFile, out); err != nil {
		return err
	}
...
func emit(source, packageName, outFile string, folds []fold) error {
	var b bytes.Buffer
