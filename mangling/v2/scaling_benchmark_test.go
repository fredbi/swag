...
// scalingTokenPool mixes plain words with default initialisms (http, id, json, api, uuid) so a swept
// input exercises the trie, folding and assembly together — IdentUnexported is the richest default path.
...
// BenchmarkGoIdentUnexportedScaling measures how IdentUnexported scales with the number of input
// tokens. The reported "ns/token" metric stays roughly constant when scaling is linear (the goal);
// a rising ns/token would flag super-linear behaviour.
